...
class SurfPosNet(nn.Module):
    ...
    def __init__(self, use_cf):
        ...
        self.deepcad_embed = nn.Sequential(
    ...
    def forward(self, surfPos, timesteps, class_label, z_data=None, is_train=False):
        """ forward pass """
        bsz = timesteps.size(0)  # bsz = 256 两块gpu
        time_embeds = self.time_embed(sincos_embedding(timesteps, self.embed_dim)).unsqueeze(1)  # time_embeds = torch.Size([256, 1, 768])
        p_embeds = self.p_embed(surfPos) # torch.Size([256, 30, 768])
        if z_data is not None:
            deepcad_embeds = self.deepcad_embed(z_data).unsqueeze(1)    
        ...
        if self.use_cf:  # classifier-free
            ...
            if z_data is not None :
                tokens = p_embeds + time_embeds + deepcad_embeds
            else:
                tokens = p_embeds + time_embeds
       #  tokens = torch.Size([256, 30, 768])
        output = self.net(src=tokens.permute(1,0,2)).transpose(0,1) # output.shape = torch.Size([256, 30, 768])
        pred = self.fc_out(output)

        return pred # torch.Size([256, 30, 6])
    
class DeepcadZNet(nn.Module):
    """
    Transformer-based latent diffusion model for deepcadz
    """    

    def __init__(self):
        super(DeepcadZNet, self).__init__()
        self.embed_dim = 64
    ...
    def forward(self, shape_code, timesteps):
        """ forward pass """
        bsz = timesteps.size(0)  # bsz = 64 两块gpu
        time_embeds = self.time_embed(sincos_embedding(timesteps, self.embed_dim)).unsqueeze(1)  # time_embeds = torch.Size([64, 1, 768])
        deepcad_embeds = self.deepcad_embed(shape_code).unsqueeze(1)  # torch.Size([64, 1, 768])  

        tokens = time_embeds + deepcad_embeds # torch.Size([64, 1, 768])
       #  tokens = torch.Size([256, 30, 768])
        output = self.net(src=tokens.permute(1,0,2)).transpose(0,1) # output.shape = torch.Size([64, 1, 768])
        pred = self.fc_out(output) # torch.Size([64, 1, 256])
        pred = pred.squeeze(1)
...
#        )
    ...
    def forward(self, surfZ, timesteps, surfPos, surf_mask, class_label, z_data=None , is_train=False):
        """ forward pass """
        bsz = timesteps.size(0) # bsz = 128

        time_embeds = self.time_embed(sincos_embedding(timesteps, self.embed_dim)).unsqueeze(1) # torch.Size([128, 1, 768])
        z_embeds = self.z_embed(surfZ) # torch.Size([128, 30, 768])
        p_embeds = self.p_embed(surfPos) # torch.Size([128, 30, 768])
...
#            num = torch.rand(1)
#            if z_data is not None and num <= 0.4:
#                deepcad_embeds = self.deepcad_embed(z_data).unsqueeze(1)
#                tokens = z_embeds + p_embeds + time_embeds + deepcad_embeds
#            else:
        ...
        ).transpose(0,1) # torch.Size([128, 30, 768])
        
        pred = self.fc_out(output) # torch.Size([128, 30, 48])
        return pred
...
class EdgePosNet(nn.Module):
    ...
    def forward(self, edgePos, timesteps, surfPos, surfZ, mask, class_label, z_data=None, is_train=False):
        """ forward pass """
        ...
        if self.use_cf:  # classifier-free
            ...
            if z_data is not None:
                deepcad_embeds = self.deepcad_embed(z_data).unsqueeze(1)
                tokens = surf_embeds + edge_p_embeds + time_embeds + deepcad_embeds
            else:
                tokens = surf_embeds + edge_p_embeds + time_embeds
#            num = torch.rand(1)
#            if z_data is not None and num <= 0.5:
#                deepcad_embeds = self.deepcad_embed(z_data).unsqueeze(1)
#                tokens = surf_embeds + edge_p_embeds + time_embeds + deepcad_embeds
#            else:
#                tokens = surf_embeds + edge_p_embeds + time_embeds
