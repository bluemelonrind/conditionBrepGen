...
class SurfPosTrainer():
    ...
    def train_one_epoch(self):
        ...
        # Train data.shape = 512, 30, 6   
        for data in self.train_dataloader:
            with torch.amp.autocast('cuda'):
                # data_cuda 是512个 30*6 的 tensor
                if self.use_cf:
                    # edit
                    ...
                    # edit
                    #surfPos, z_data = data_cuda
                    data_cuda = [dat.to(self.device) for dat in data]
                    surfPos, z_data = data_cuda
                    class_label = None
                # surfPos.shape = (512, 30, 6)

                bsz = len(surfPos)
                
                # Augment the z_data (see https://arxiv.org/abs/2106.15282)
                conditions = [z_data]
                aug_data = []
                for data in conditions:
                    aug_timesteps = torch.randint(0, 15, (bsz,), device=self.device).long() # 256
                ...
                # Add noise, timesteps.shpae = 512
                timesteps = torch.randint(0, self.noise_scheduler.config.num_train_timesteps, (bsz,), device=self.device).long()  # [batch,]
                surfPos_noise = torch.randn(surfPos.shape).to(self.device)  # surfPos_noise.shape torch.Size([512, 30, 6])
                surfPos_diffused = self.noise_scheduler.add_noise(surfPos, surfPos_noise, timesteps)  # torch.Size([512, 30, 6])

                # Predict noise
                surfPos_pred = self.model(surfPos_diffused, timesteps, class_label, z_data, True)
...
class DeepcadZTrainer():
    """Deepcad latent encode Trainer """
    def __init__(self, args, train_dataset, val_dataset): 
        # Initilize model and load to gpu
        self.iters = 0
        self.epoch = 0
        ...
        # Initialize wandb
        wandb.init(project='deepcadz_ldm', dir=self.save_dir, name=args.env)
    ...
    def train_one_epoch(self):
        ...
        for data in self.train_dataloader:
            with torch.amp.autocast('cuda'):
                ...
                shape_code = data.to(self.device) #torch.Size([128, 256])

                bsz = len(shape_code) #bsz = 128
                
                self.optimizer.zero_grad() # zero gradient

                # Add noise, timesteps.shpae = 512
                timesteps = torch.randint(0, self.noise_scheduler.config.num_train_timesteps, (bsz,), device=self.device).long()  # [batch,] 128
                shape_code_noise = torch.randn(shape_code.shape).to(self.device)  #torch.Size([128, 256])
                shape_code_diffused = self.noise_scheduler.add_noise(shape_code, shape_code_noise, timesteps)  # torch.Size([128, 256])

                # Predict noise
                shape_code_pred = self.model(shape_code_diffused, timesteps) # torch.Size([128, 256])
              
                # Compute loss
                total_loss = self.loss_fn(shape_code_pred, shape_code_noise)
    ...
    def test_val(self):
        ...
        for data in self.val_dataloader:
            shape_code = data.to(self.device)

            bsz = len(shape_code)
        
            total_count += len(shape_code)
            
            for idx, step in enumerate([10,50,100,200,500]):
                # Evaluate at timestep 
                timesteps = torch.randint(step-1, step, (bsz,), device=self.device).long()  # [batch,]
                shape_code_noise = torch.randn(shape_code.shape).to(self.device)  
                shape_code_diffused = self.noise_scheduler.add_noise(shape_code, shape_code_noise, timesteps)
                with torch.no_grad():
                    shape_code_pred = self.model(shape_code_diffused, timesteps) 
                loss = mse_loss(shape_code_pred, shape_code_noise).mean((1,2)).sum().item()
                total_loss[idx] += loss
        ...
        for data in self.train_dataloader:
            with torch.amp.autocast('cuda'):
                ...
                if self.use_cf:
                    ...
                    surfPos, surfPnt, surf_mask, z_data = data_cuda  
                    class_label = None # surfPos (256, 30, 6) /surfPnt (256, 30, 32, 32, 3)/ surf_mask (256, 30)/ z_data (256, 256)

                bsz = len(surfPos) # bsz = 256

                # Augment the surface position (see https://arxiv.org/abs/2106.15282)
                conditions = [surfPos, z_data]
                aug_data = []
                for data in conditions:
                    aug_timesteps = torch.randint(0, 15, (bsz,), device=self.device).long() # 256
                    aug_noise = torch.randn(data.shape).to(self.device)  # aug_noise.shape (256, 30, 6)
                    aug_data.append(self.noise_scheduler.add_noise(data, aug_noise, aug_timesteps))
                surfPos, z_data = aug_data[0], aug_data[1] # surfPos (256, 30, 6)

                # Pass through surface VAE to sample latent z 
                with torch.no_grad():
                    surf_uv = surfPnt.flatten(0,1).permute(0,3,1,2) # surf_uv.shape (torch.Size([7680, 3, 32, 32]))
                    surf_z = self.surf_vae(surf_uv) # torch.Size([7680, 3, 4, 4])
                    surf_z = surf_z.unflatten(0, (bsz, -1)).flatten(-2,-1).permute(0,1,3,2)     # torch.Size([256, 30, 16, 3])

                surfZ = surf_z.flatten(-2,-1)  * self.z_scaled # rescaled the latent z    torch.Size([256, 30, 48])
                
                self.optimizer.zero_grad() # zero gradient

                # Add noise
                timesteps = torch.randint(0, self.noise_scheduler.config.num_train_timesteps, (bsz,), device=self.device).long()  # [batch,] 256
                surfZ_noise = torch.randn(surfZ.shape).to(self.device)   # torch.Size([256, 30, 48])
                surfZ_diffused = self.noise_scheduler.add_noise(surfZ, surfZ_noise, timesteps) # torch.Size([256, 30, 48])

                # Predict noise
                surfZ_pred = self.model(surfZ_diffused, timesteps, surfPos, surf_mask, class_label, z_data, True)
        ...
        for data in self.train_dataloader:
            with torch.amp.autocast('cuda'):
                ...
                if self.use_cf:
                    ...
                    edgePos, surfPnt, surfPos, surf_mask, z_data = data_cuda
                    class_label = None
                bsz = len(surfPos)
                ...
                conditions = [surfPos, surfZ, z_data]
                aug_data = []
                for data in conditions:
                    aug_timesteps = torch.randint(0, 15, (bsz,), device=self.device).long()
                    aug_noise = torch.randn(data.shape).to(self.device)  
                    aug_data.append(self.noise_scheduler.add_noise(data, aug_noise, aug_timesteps))
                surfPos, surfZ, z_data = aug_data[0], aug_data[1], aug_data[2]
                ...
                # Predict noise
                edgePos_pred = self.model(edgePos_diffused, timesteps, surfPos, surfZ, surf_mask, class_label, z_data, True)
    ...
    def test_val(self):
        ...
        for data in self.val_dataloader:
            ...
            for idx, step in enumerate([10,50,100]):
                ...
                with torch.no_grad():
                    pred = self.model(diffused, timesteps, surfPos, surfZ, surf_mask, class_label, z_data)
                loss = mse_loss(pred[~surf_mask], noise[~surf_mask]).mean(-1).sum().item()
                total_loss[idx] += loss
