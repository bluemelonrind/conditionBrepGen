...
def filter_data(data):
    ...
    if not os.path.exists(data_path):
        print(f"[警告] 文件 {data_path} 不存在，跳过")
        return None, None
...
def load_data(input_data, input_list, validate, args):
    ...
    #     convert_iter = pool.apply_async(filter_data, (params[0],))
...
# 找到每一个批量的文件索引，根据/workspace/data/personal_data/yzf/deepcad/z_dict.pkl
# z_dict 存储的是"0067/00675619"：256维向量，字典数据结构
# data_process/deepcad_parsed/0000/00005958.pkl，SurfPosData.data 里面是这种格式
...
class SurfPosData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        z_path = os.path.join(data_path.split('.')[0].split('/')[2], data_path.split('.')[0].split('/')[3])
        #with open("/workspace/data/personal_data/yzf/deepcad/z_dict.pkl", "rb") as f:
        #   z_dict = pickle.load(f)
        z_data = self.z_dict[z_path]
        ...
        if data_class is not None:
            ...
            return (torch.FloatTensor(surf_pos), torch.FloatTensor(z_data))# abc or deepcad
            #return torch.FloatTensor(surf_pos)

class DeepcadZData(torch.utils.data.Dataset):
    """Deepcad latent encode Dataloader """
    def __init__(self, validate=False):
            with h5py.File('/workspace/data/personal_data/yzf/DeepCAD/proj_log/newDeepCAD/results/all_zs_ckpt1000.h5', 'r') as fp:
                if validate:
                    self.data = fp["validation_zs"][:]
                else:
