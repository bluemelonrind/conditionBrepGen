...
def sample(eval_args, i):
    ...
    #deepcadz_model = DeepcadZNet()
    #deepcadz_model.load_state_dict(torch.load(eval_args['deepcadz_weight']))
    #deepcadz_model = deepcadz_model.to(device).eval()
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            ...
            ###########################################
            # STEP 0: generate the deepcadz #
            ###########################################
            #shape_code = randn_tensor((batch_size, 256)).to(device)

            #pndm_scheduler.set_timesteps(50)   
            #for t in tqdm(pndm_scheduler.timesteps): 
            #    timesteps = t.reshape(-1).cuda()
            #    pred = deepcadz_model(shape_code, timesteps)
            #    shape_code = pndm_scheduler.step(pred, t, shape_code).prev_sample
            
            #ddpm_scheduler.set_timesteps(1000)  
            #for t in tqdm(ddpm_scheduler.timesteps[-500:]):   
            #    timesteps = t.reshape(-1).cuda()
            #    pred = deepcadz_model(shape_code, timesteps)
            #    shape_code = ddpm_scheduler.step(pred, t, shape_code).prev_sample

            # add lgan generated deepcadz
            #with h5py.File('/workspace/data/personal_data/yzf/deepcad/proj_log/newDeepCAD/lgan_1000/results/1fake_z_ckpt200000_num9000.h5', 'r') as f:
            #    zs = f['zs'][:]
            with open("/workspace/data/personal_data/yzf/deepcad/z_dict.pkl", "rb") as f:
                z_dict = pickle.load(f)
            z_path = '0043/00435622'
            z = z_dict[z_path]
            
            # 取batch_size个
            #batch_z = torch.tensor(zs[i*batch_size:(i+1)*batch_size], dtype=torch.float32).to(device)
            #batch_z = torch.tensor(np.array([zs[8]] * batch_size), dtype=torch.float32).to(device)
            batch_z = torch.tensor(np.array([z] * batch_size), dtype=torch.float32).to(device)
            ###########################################
            ...
            surfPos = randn_tensor((batch_size, num_surfaces, 6), generator=gen, device=device)
            ...
            for t in tqdm(pndm_scheduler.timesteps[:158]):#
                ...
                if class_label is not None:
                    ...
                    pred = surfPos_model(surfPos, timesteps, class_label, batch_z)
    ...
    for batch_idx in range(batch_size):
        ...
        write_stl_file(solid, f'{save_folder}/{random_string}_{i}_{batch_idx}.stl', linear_deflection=0.001, angular_deflection=0.5)
    return 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu", type=int, nargs='+', default=[2], help="GPU IDs to use for training")
    args = parser.parse_args()    

    # Load evaluation config 
    with open('/workspace/data/personal_data/yzf/BrepGen/eval_config.yaml', 'r') as file:
